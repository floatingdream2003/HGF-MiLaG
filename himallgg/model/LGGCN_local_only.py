...
class LGGCN(nn.Module):
    ...
    def get_rep(self, data):
        ...
        ##################################################################################################################
        return graph_out_T, graph_out_A, graph_out_V

    # 前向传播
    def forward(self, data):
        graph_out_T, graph_out_A, graph_out_V = self.get_rep(data)
        ...
        scores = 0.7 * score_T + 0.2 * score_V + 0.1 * score_A

        # 性别分类

        score_gT = self.clf_gT.get_prob1_gender(graph_out_T, data["text_len_tensor"])
        score_gA = self.clf_gA.get_prob1_gender(graph_out_A, data["text_len_tensor"])
        score_gV = self.clf_gV.get_prob1_gender(graph_out_V, data["text_len_tensor"])
        scores_g = 0.7*score_gT+0.2*score_gA+0.1*score_gV
    ...
    def get_loss(self, data):
        ...
        loss_gT = self.clf_gT.get_loss(graph_out_T, data["xingbie_tensor"], data["text_len_tensor"])
        loss_gA = self.clf_gA.get_loss(graph_out_A, data["xingbie_tensor"], data["text_len_tensor"])
        loss_gV = self.clf_gV.get_loss(graph_out_V, data["xingbie_tensor"], data["text_len_tensor"])


        loss_emotion = 0.7 * loss_T + 0.2 * loss_V + 0.1 * loss_A
        loss_gender = 0.7 * loss_gT + 0.2 * loss_gA + 0.1 * loss_gV
        # loss_gender = loss_g
